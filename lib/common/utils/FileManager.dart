...
class FileManager {
  ...
  /// 获取本地文件
  File loadFileByPath(String filePath) {
    // 处理fileName
    String char = filePath.substring(0, 1);
    if (char == "/") {
  ...
  /// 读取文件路径列表
  List<String> loadFileList({String headerPath = "cacheFile"}) {
  ...
  /// 写入文件数据字符串
  Future<File> writeAsString(String fileName, String content,
  ...
  Future<List<Uint8List>> readAsUint8List(List<String> filePathList,
    ...
    for (String fileName in filePathList) {
      File file = loadFileByPath(fileName);
  ...
  /// 同步批量获取二进制文件数据
  List<Uint8List> readAsUint8ListSync(List<String> filePathList,
      {String headerPath = "cacheFile"}) {
    List<Uint8List> result = [];
    for (String fileName in filePathList) {
      File file = loadFileByPath(fileName);
  ...
  /// 批量删除文件
  void deleteFileList(List<String> filePathList) {
    for (String fileName in filePathList) {
      File file = loadFileByPath(fileName);
      file.delete();
    }
